  ...
  sapply(function(x){
    bestmod_list %>%
      ...
      sapply(X=.,Y=x,function(X,Y){
        Z=bestmod_list$blood_t[bestmod_list$grank==X & bestmod_list$rank==Y] %>%
          .[!duplicated(.)]
...
eligible_biomarkers %>%
  mutate(
    mod_list=
      ifelse(
        mod_list%in%c('logFC2_mod','devnorep3DEG_mod')
        ,mod_list#'logFC2_mod_devnorep3DEG_mod'
        ,mod_list
      )
  ) %>%
  lapply(X=1,Y=.,function(X,Y){
  ...
  mutate(
    mod_list=
      case_when(
        mod_list=='bestmod_mod'~'blood-derived surrogate transcriptome'
        ,mod_list=='logFC2_mod'~'blood transcriptome, p.val<.05 & abs(logFC)>2'
        ,mod_list=='devrep3DEG_mod'~'blood transcriptome, p.val<.05 both train & test'
        ,mod_list=='devnorep3DEG_mod'~'blood transcriptome, p.val<.05 train but not test'
        ,mod_list=='recentDEG_mod'~'blood transcriptome, a recent study'
        ,mod_list=='logFC2_mod_devnorep3DEG_mod'~'blood transcriptome, p.val<.05 train but not test'
        ,TRUE~''
      )
  )
```
...
if(show_best_tree){
  ...
  best_each_method$mod %>%
    .$trainingData %>%
    rename(outcome=.outcome) %>%
    tree_eval(best_each_method$mod,best_each_method$eval)
}
```
...
if(show_best_tree){
  bestmod_list %>%
      ...
      %in%str_split(
        eligible_biomarkers %>%
          filter(
            cand_predictor
            %in%paste0(
              best_each_method$mod$trainingData %>%
                colnames() %>%
                .[.!='.outcome'] %>%
                sort()
              ,collapse=','
    ...
    rbind(
      left_join(.,bestmod_list,by='gene') %>%
        filter(
          tissue.x!=tissue.y
          & blood_t.x==blood_t.y
        ) %>%
...
if(show_best_tree){
  bestmod_raw_weights %>%
    left_join(bestmod_weights,c('tissue','blood_t')) %>%
    left_join(bestmod_list,c('tissue','gene','blood_t','avg_accu_w','rank')) %>%
  ...
  mutate(
    value=
      ...
      %in%pull(
        protein_exprs %>%
          rbind(
            rbind(mirna_target_reg_pe,mirna_target_reg_fgrpe) %>%
              filter(
