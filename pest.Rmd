...
```{r}
timestamper=c()
timestamper['0. Pre-start']=format(Sys.time(),'%Y-%m-%d %H:%M:%S')

memstamper=c()
memstamper['0. Pre-start']=memory.size()
```
...
```{r}
timestamper['1. Start']=as.character(now())
memstamper['1. Start']=memory.size()
```
...
```{r}
timestamper['2. Phenotype data']=as.character(now())
memstamper['2. Phenotype data']=memory.size()
```
...
```{r}
timestamper['3. Expression data']=as.character(now())
memstamper['3. Expression data']=memory.size()
```
...
```{r}
timestamper['4. Feature data']=as.character(now())
memstamper['4. Feature data']=memory.size()
```
...
```{r}
timestamper['5. A01 Outlier and low expression removal']=as.character(now())
memstamper['5. A01 Outlier and low expression removal']=memory.size()
```
...
```{r}
timestamper['5. A02 Outlier and low expression removal']=as.character(now())
memstamper['5. A02 Outlier and low expression removal']=memory.size()
```
...
```{r}
timestamper['5. A03 Outlier and low expression removal']=as.character(now())
memstamper['5. A03 Outlier and low expression removal']=memory.size()
```
...
```{r}
timestamper['5. A04 Outlier and low expression removal']=as.character(now())
memstamper['5. A04 Outlier and low expression removal']=memory.size()
```
...
```{r}
timestamper['5. Outlier and low expression removal']=as.character(now())
memstamper['5. Outlier and low expression removal']=memory.size()
```
...
```{r}
timestamper['6. Gene summarization']=as.character(now())
memstamper['6. Gene summarization']=memory.size()
```
...
```{r}
timestamper['7. Data exploration of common genes']=as.character(now())
memstamper['7. Data exploration of common genes']=memory.size()
```
...
```{r}
timestamper['8. Derivation, population-level DEGs']=as.character(now())
memstamper['8. Derivation, population-level DEGs']=memory.size()
```
...
```{r}
timestamper['9. Derivation, individual-level DEGs']=as.character(now())
memstamper['9. Derivation, individual-level DEGs']=memory.size()
```
...
```{r}
timestamper['10. Individual DEGs of development dataset']=as.character(now())
memstamper['10. Individual DEGs of development dataset']=memory.size()
```
...
```{r}
timestamper['11a. Non-DI-VNN model development']=as.character(now())
memstamper['11a. Non-DI-VNN model development']=memory.size()
```
...
```{r}
timestamper['11b. DI-VNN model development']=as.character(now())
memstamper['11b. DI-VNN model development']=memory.size()
```

```{r Create a function to compare calibration of the models, include=FALSE}
source('R/calib_comparison-function.R')
```
...
```{r}
timestamper['12. Model evaluation']=as.character(now())
memstamper['12. Model evaluation']=memory.size()
```
...
```{r}
timestamper['13. A01 Biomarker emulation']=as.character(now())
memstamper['13. A01 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A02 Biomarker emulation']=as.character(now())
memstamper['13. A02 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A03 Biomarker emulation']=as.character(now())
memstamper['13. A03 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A04 Biomarker emulation']=as.character(now())
memstamper['13. A04 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A05 Biomarker emulation']=as.character(now())
memstamper['13. A05 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A06 Biomarker emulation']=as.character(now())
memstamper['13. A06 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A07 Biomarker emulation']=as.character(now())
memstamper['13. A07 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A08 Biomarker emulation']=as.character(now())
memstamper['13. A08 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A09 Biomarker emulation']=as.character(now())
memstamper['13. A09 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A10 Biomarker emulation']=as.character(now())
memstamper['13. A10 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A11 Biomarker emulation']=as.character(now())
memstamper['13. A11 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A12 Biomarker emulation']=as.character(now())
memstamper['13. A12 Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['13. A13 Biomarker emulation']=as.character(now())
memstamper['13. A13 Biomarker emulation']=memory.size()
```
...
if(load_emulators){
  ...
  saveRDS(tmp3j,'data/tmp3j.rds')
}else{
  tmp3j=readRDS('data/tmp3j.rds')
}
```

```{r}
timestamper['13. A14 Biomarker emulation']=as.character(now())
memstamper['13. A14 Biomarker emulation']=memory.size()
```
...
if(load_emulators){
  ...
  saveRDS(tmp3k,'data/tmp3k.rds')
}else{
  tmp3k=readRDS('data/tmp3k.rds')
}
```

```{r}
timestamper['13. A15 Biomarker emulation']=as.character(now())
memstamper['13. A15 Biomarker emulation']=memory.size()
```
...
if(load_emulators){
  saveRDS(tmp3e$rank2$grank3,'data/tmp3e_rank2_grank3.rds')
}else{
  tmp3e_rank2_grank3=readRDS('data/tmp3e_rank2_grank3.rds')
}
...
if(load_emulators){
  saveRDS(tmp3d$rank2$grank3,'data/tmp3d_rank2_grank3.rds')
}else{
  tmp3d_rank2_grank3=readRDS('data/tmp3d_rank2_grank3.rds')
}
...
```{r}
timestamper['13. A16 Biomarker emulation']=as.character(now())
memstamper['13. A16 Biomarker emulation']=memory.size()
```

```{r Create a function to evaluate leaves of a biomarker tree, include=FALSE}
source('R/tree_eval-function.R')
```
...
```{r The surrogate transcriptome in the best model (any ranks), echo=FALSE}
if(show_best_tree){
  tmp3 %>%
...
```{r The best tree (training PE), eval=FALSE, fig.height=3, fig.width=3, include=FALSE}
if(show_best_tree){
  tmp3j$mod %>%
    .$finalModel %>%
    rpart.plot(
...
```{r The best tree (training FGR), eval=FALSE, fig.height=3, fig.width=3, include=FALSE}
if(show_best_tree){
  tmp3j$mod %>%
    .$finalModel %>%
    rpart.plot(
...
```{r}
timestamper['13. Biomarker emulation']=as.character(now())
memstamper['13. Biomarker emulation']=memory.size()
```
...
```{r}
timestamper['14. Interpretation with miRNA']=as.character(now())
memstamper['14. Interpretation with miRNA']=memory.size()
```
...
```{r}
timestamper['15. Interpretation with p-p interaction']=as.character(now())
memstamper['15. Interpretation with p-p interaction']=memory.size()
```
...
```{r}
timestamper['16. Interpretation with PTM']=as.character(now())
memstamper['16. Interpretation with PTM']=memory.size()
```

```{r}
timestamper %>%
  data.frame(seq=seq(0,length(.)-1),timestamp=.) %>%
  rownames_to_column(var='label') %>%
  mutate(label=factor(label,rev(unique(label)))) %>%
...
big_files=
  ...
  mutate(
    ...
    ,file=
      path %>%
      str_sub(file+1,str_length(path))
  )

# file.rename(big_files$path,paste0('../pest_drive/',big_files$file))
# saveRDS(big_files,'data/big_files.rds')
```

```{r}
memstamper %>%
  data.frame(seq=seq(0,length(.)-1),memstamp=.) %>%
  rownames_to_column(var='label') %>%
  mutate(label=factor(label,rev(unique(label)))) %>%
  mutate(memstamp=memstamp-min(memstamp)) %>%
  left_join(
    mutate(.,seq=seq+1) %>%
      rename(memstamp0=memstamp) %>%
      select(-label)
    ,by='seq'
  ) %>%
  slice(-1) %>%
  mutate(memory_size=memstamp-memstamp0) %>%
  select(-memstamp0) %>%
  gather(metric,value,-label,-seq) %>%
  ggplot(aes(label,value)) +
  geom_col() +
